...
def main() -> int:
    ...
    assert len(rows) == 144, f"expected 144 pulled issues, found {len(rows)}"
    assert max(by_issue) == 164
    assert sum(row["state"] == "open" for row in rows) == 140
    ...
    assert (ROOT / "VERSION.txt").read_text(encoding="utf-8").strip() == "2.4"
