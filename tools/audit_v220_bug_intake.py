...
def main() -> int:
    ...
    assert len(rows) == 129, f"expected 129 pulled issues, found {len(rows)}"
    assert max(by_issue) == 149
    assert sum(row["state"] == "open" for row in rows) == 125
    ...
    assert (ROOT / "VERSION.txt").read_text(encoding="utf-8").strip() == "2.2"
