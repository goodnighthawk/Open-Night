...
def main() -> None:
    ...
    assert len(lamps) == 87, "only the three Hudson-channel lamp anchors should be removed"
    ...
    assert not lane_dividers, "v2.8 displaced thick white divider bars returned"
    assert world.data.get("runtime_refinement", {}).get("six_lane_divider_policy") == "removed_thick_off_cell_bars_v28"
