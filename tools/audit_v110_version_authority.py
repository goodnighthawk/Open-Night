...
def main() -> None:
    ...
    require(version_label() == f"Open Night v{EXPECTED}", version_label())
    ...
    require(f"PYMMO_PATCH_ID=open-night-v{EXPECTED}" in railway,
            f"Railway patch id is not v{EXPECTED}")
    ...
    require(f"server_name,Open Night v{EXPECTED}" in server_config,
            f"repo-local server default is not v{EXPECTED}")
    require(f"OPEN NIGHT v{EXPECTED}" in read("RUN_CLIENT.bat"),
            "desktop client launcher label is stale")
    run_server = read("RUN_SERVER.bat")
    require(f"OPEN NIGHT v{EXPECTED}" in run_server,
    ...
    audit = {
        ...
        "railway_patch_id": f"open-night-v{EXPECTED}",
