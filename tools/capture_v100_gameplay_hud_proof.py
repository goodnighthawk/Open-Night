...
# After the true 0.5x world normalization, return toward normal gameplay zoom.
# This keeps the player readable while still exposing multiple GTA2-like blocks.
PROOF_ZOOM = 0.72
...
def main() -> None:
    ...
    if game.grid_world.cell_px != v100_scale_normalization.TARGET_CELL_PX:
        raise RuntimeError(f"runtime proof expected 128px normalized cells, got {game.grid_world.cell_px}")
    ...
    proof_fires = shifted_fires or fires
    if not proof_fires or not lamps:
    ...
    choices = [(cell_distance(fire, lamp), fire, lamp) for fire in proof_fires for lamp in lamps if cell_distance(fire, lamp) > 0]
    ...
    local = game_client.RemotePlayer({
        "id": "runtime-proof-local", "name": "RuntimeProof", "x": x, "y": y,
        "aim": -1.5707963267948966, "cash": 420, "packages": 2,
    ...
    game.notice = "v1.0 normalized proof — 0.5x world scale, curb-safe buildings, player + HUD + M map"
    ...
    audit = {
        "proof": "canonical_v100_normalized_gameplay_hud_and_m_map",
        ...
        "m_map_capture": True,
        "review_board_panels": ["normalized_gameplay_hud", "actual_M_map", "full_ground_overview"],
        "proportion_reference": "GTA2-like overhead framing with player retained at existing readable scale",
        "proof_target": {
    ...
    if refinement.get("normalized_cell_px") != 128 or refinement.get("world_linear_scale") != 0.5:
        raise RuntimeError(f"world-scale normalization audit failed: {refinement}")
