...
class EnvironmentRenderer:
    ...
    def _draw_roof_modules(self, surface: pygame.Surface, rect: pygame.Rect, inner: pygame.Rect, index: int, elevated_25d: bool, module_count: int | None = None) -> None:
        ...
        detail_count = max(2, min(7, area // 3200 + 1)) if elevated_25d else max(1, min(4, area // 5000 + 1))
    ...
    def _draw_chunk_water(self, surface: pygame.Surface, cx: int, cy: int, ox: int, oy: int) -> None:
        for poly in self._water_index.get((cx, cy), ()):
            local = [(int(p[0] - ox), int(p[1] - oy)) for p in poly]
            if len(local) >= 3:
                self._texture_masked_polygon(surface, local, _approved_env_tile("water_64"), WATER_COLOR)
    ...
    def _draw_chunk_roads(self, surface: pygame.Surface, cx: int, cy: int, ox: int, oy: int) -> None:
        ...
        for road in self._road_index.get((cx, cy), ()):
            ...
            furnishing_side = 10 if sidewalk_side >= 20 else 0
            frontage_side = 8 if sidewalk_side >= 20 else 0
        ...
        self._paint_geometry_mask(surface, sidewalk_geom, _approved_env_tile("sidewalk_64"), SIDEWALK_COLOR)
        ...
        self._paint_geometry_mask(surface, road_union, _approved_env_tile("asphalt_64"), ROAD_COLOR)
    ...
    def _draw_chunk_street_props(self, surface: pygame.Surface, cx: int, cy: int, ox: int, oy: int) -> None:
        ...
        for prop in self._street_prop_index.get((cx, cy), ()):
            ...
            sprite = _approved_prop(kind, max(12, int(round(base_h * scale))))
            if sprite is None:
                continue
            rotation = float(prop.get("rotation", 0.0))
    ...
    def _render_chunk(self, cx: int, cy: int) -> pygame.Surface:
        ...
        # Street furniture is authored after buildings so trees/lamps read clearly
        # along the pavement edge in the same visual layer as the approved target.
    ...
    def set_map(self, map_config: dict) -> None:
        ...
        signature = (map_id, int(map_config.get("world_w", 1)), int(map_config.get("world_h", 1)), bool(map_config.get("chunked")))
