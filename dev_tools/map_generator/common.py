...
def point_near_road(x: float, y: float, map_config: dict, extra: float = 0.0, *, bridge_only: bool = False) -> bool:
    grid = map_config.get("_grid")
    if grid is not None:
...
def blocked(x: float, y: float, map_config: dict | None = None) -> bool:
    ...
    if cfg.get("chunked") and water_candidate and point_in_water(x, y, cfg) and not point_near_road(x, y, cfg, extra=PLAYER_RADIUS, bridge_only=True):
        return True
...
def move_with_collisions(
    ...
    nx = x + dx
    if not blocked(nx, y, map_config):
        x = nx

    ny = y + dy
    if not blocked(x, ny, map_config):
        y = ny
