...
scale_history: dict[tuple[str, int], list[float]] = defaultdict(list)
...
def choose_building_sprite(district: str, width: float, height: float, sequence: int, landmark: bool = False) -> dict:
    ...
    def scale_safe(item) -> bool:
        key = (item[1], item[2])
        values = scale_history[key]
        if not values:
            return True
        render = item[6]
        return max(max(values), render) - min(min(values), render) <= 0.10 + 1e-9

    safe_shortlist = [item for item in shortlist if scale_safe(item)]
    if not safe_shortlist:
        # A slightly less native candidate is preferable to making one repeated
        # atlas cell impersonate buildings at visibly different scales.
        safe_shortlist = [
            item for item in evaluated
            if item[0] <= best_native + 0.16
            and item[5] >= base.MIN_BUILDING_SPRITE_SCALE
            and scale_safe(item)
        ]
    if safe_shortlist:
    ...
    scale_history[key].append(render)
    return {
