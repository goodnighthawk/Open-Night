...
`LOCAL_QA.bat` runs 21 repeatable asset, map-scale, bicycle-water, multiplayer, social, collision, traffic, multi-level, launcher, internet-discovery and portable-map checks. `tools\pass38_playability_smoke.py` adds a real local server + portable-map protocol gate.
...
On-foot WASD is camera-relative. Vehicle throttle/steering remains vehicle-relative. Space jumps, C crouches, T enters/drives/rides/exits vehicles, E interacts/trades/enters rooms, Enter chats, mouse head-look remains client-side cosmetic state, middle-mouse rotates the camera, and the mouse wheel zooms. Esc opens the scrollable Settings and Friends pages.
